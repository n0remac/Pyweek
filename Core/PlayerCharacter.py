...
from Constants.Game import SPRITE_SCALING_PLAYER, DOWN_FACING, UP_FACING, RIGHT_FACING, LEFT_FACING
...
class PlayerCharacter(arcade.Sprite):
    ...
    def __init__(self):
        ...
        self.down_idle_texture_pair = arcade.load_texture_pair(f"{main_path}_down_idle0.png")
        self.down_walk_texture_pair = arcade.load_texture_pair(f"{main_path}_down_walk0.png")
        self.up_idle_texture_pair = arcade.load_texture_pair(f"{main_path}_up_idle0.png")
        self.up_walk_texture_pair = arcade.load_texture_pair(f"{main_path}_up_walk0.png")
        ...
        if self.sub_texture > (60/self.frames):
            ...
            if self.change_x == 0 and self.change_y == 0:
                ...
                if self.character_face_direction == DOWN_FACING:
                    self.texture = self.down_idle_textures[self.cur_texture][self.character_face_direction_horizontal]
                    return
                else:
                    self.texture = self.up_idle_textures[self.cur_texture][self.character_face_direction_horizontal]
                return
            else:
                # Walking animation
                ...
                if self.cur_texture > (self.frames-2):
                    self.cur_texture = 0
                if self.character_face_direction == DOWN_FACING:
                    self.texture = self.down_walk_textures[self.cur_texture][self.character_face_direction_horizontal]
                    return
                else:
                    self.texture = self.up_walk_textures[self.cur_texture][self.character_face_direction_horizontal]
