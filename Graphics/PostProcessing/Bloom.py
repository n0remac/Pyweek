...
class Bloom(PostEffect):
    ...
    def on_add(self, post_processing_chain, context, window_size):
        ...
        self.rt_half_ping = RenderTarget(context, self.get_half_rt_size(window_size), 'f2')
        self.rt_half_pong = RenderTarget(context, self.get_half_rt_size(window_size), 'f2')

        self.rt_quater_ping = RenderTarget(context, self.get_quater_rt_size(window_size), 'f2')
        self.rt_quater_pong = RenderTarget(context, self.get_quater_rt_size(window_size), 'f2')
    ...
    def get_half_rt_size(self, window_size):
        return (int(math.ceil(window_size[0] / 2.0)), int(math.ceil(window_size[1]) / 2.0))

    def get_quater_rt_size(self,window_size):
        return (int(math.ceil(window_size[0] / 4.0)), int(math.ceil(window_size[1]) / 4.0))
